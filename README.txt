...
o	Go to the Watson Assistant page in the IBM Cloud Catalog.
...
o	If your service instance uses username and password credentials, add the ASSISTANT_USERNAME and ASSISTANT_PASSWORD variables to the .env file.
...
o	Go to the AWS sevices EC2 page in the AWS Cloud Services.
o	Log in to your AWS Cloud account.
o	Click  Services.
o	Click EC2 under compute to create an instance.
o	Click Launch Instance
o	Choose an AMI (eg: Ubuntu Server 16.04 LTS (HVM), SSD Volume Type - ami-0f9cf087c1f27d9b1 (64-bit x86) / ami-036ede09922dadc9b (64-bit Arm) I have used this for my project)
o	Click select and choose a instance type (t2 micro is used in this project).
o	Click next until you see launch button.
o	Select a new key-pair and download the keypair immediately later this file won't be available and click create
o	And the Instance is created and then moves to running state
o	Copy the Public DNS and Public IP and store it for later use.
###Running the application in AWS cloud:
In the terminal do the following:
o	Run command chmod 400 ./aws-keypair.pem (./aws-keypair.pem is the one which you download while creating the instance)
o	Run  comman ssh -i /aws-keypair.pem ubuntu@"public DNS" (paste the previously copied public DNS value in "public DNS" in your command).
o	Now you will enter into aws cloud.
o	Clone the project into the aws cloud.
o	Inside the cloned project now copy the .env.example file to .env
Example .env file that configures the username, password, and url for a Watson Assistant service instance hosted in the US South region:
ASSISTANT_USERNAME=apikey
ASSISTANT_PASSWORD=dtkuu77989gygkyyt6g
ASSISTANT_URL=https://gateway.watsonplatform.net/assistant/api
...
ASSISTANT_IAM_APIKEY=ca2905e6 ttMEkgoiN76NuhS_fyuy78jhhyhhgfyu
ASSISTANT_IAM_URL=https://gateway-syd.watsonplatform.net/assistant/api
10.	Add the WORKSPACE_ID to the previous properties
11.	WORKSPACE_ID=522be-0hjj-ab44-dec3-g1eab2ha73c68gu
...
o	Under Network and Security go to Security Groups in AWS cloud 
o	Select a security group other than default
 
o	Under inbound click edit and click add rule set the type to Custom TCP Rule, port range( give ibm port number here i.e: 3000) 
o	Click Save 
o	Now with the public IP (previously copied information while creating an instance) of the instance in the aws cloud and port-number 3000 from ibm cloud check whether the hybrid cloud is deployed successfully
